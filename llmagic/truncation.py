...
def process_boundary_points(boundary_points: list[int], max_tokens: int, size: int = None, direction="right") -> int:
    if boundary_points is not None:

        if direction == "left":
            while (size - max_tokens - 1) not in boundary_points and (size - max_tokens - 1) > 0:
                max_tokens -= 1
        elif direction == "right":
...
def truncate(
    ...
    if max_tokens is not None and size > max_tokens:
        match truncation_strategy:
            case "right":
                processed_max_tokens = process_boundary_points(boundary_points, max_tokens, direction="right")
                cutoff = size - processed_max_tokens
                remainder_right = truncate_encoding(tokens, cutoff, direction="left")
                tokens = truncate_encoding(tokens, processed_max_tokens, direction="right")
                if ellipsis:
                    tokens = add_ellipsis_token(tokens, ellipsis_token=ellipsis_tokens, direction="right")
            case "left":
                processed_max_tokens = process_boundary_points(boundary_points, max_tokens, size, direction="left")
                cutoff = size - processed_max_tokens
                remainder_left = truncate_encoding(tokens, cutoff, direction="right")
                tokens = truncate_encoding(tokens, processed_max_tokens, direction="left")
                if ellipsis:
                    tokens = add_ellipsis_token(tokens, ellipsis_token=ellipsis_tokens, direction="left")
            case _:
                # No truncation
                pass
    return {
